...
use gtk::{
    ...
    glib::{self, get_user_data_dir, prelude::*, subclass::Signal},
};
use once_cell::sync::Lazy;
use std::{cell::RefCell, collections::HashMap};
use toml::de::Error;

static mut USER_CONFIG_MANAGER: Option<UserConfig> = None;
...
mod imp {
    ...
    impl ObjectImpl for UserConfig {
...
glib::wrapper! {
    pub struct UserConfig(ObjectSubclass<imp::UserConfig>);
}
...
impl UserConfig {
    ...
    fn new() -> Self {
        ...
        match get_user_data_dir().as_os_str().to_str() {
            Some(user_data_directory) => {
                ...
                let self_ = &mut imp::UserConfig::from_instance(&object);
    ...
    pub fn get_map(&self) -> HashMap<String, VaultConfig> {
        let self_ = &mut imp::UserConfig::from_instance(&self);
    ...
    pub fn get_current_vault(&self) -> Option<Vault> {
        let self_ = imp::UserConfig::from_instance(&self);
